...
The website build generates `/registry/models-v1.json`, available at `https://your-domain/registry/models-v1.json` after deployment. It uses `packages/provider-registry/models.json` (candidates) and `providers.json` (connection and model capability presets), with a schema version, data version and content hash. It contains no user configurations or API keys. The app and website share `packages/provider-registry/version.json`, initially set to `1`. Update the model source files, increment its `version`, and deploy the website to refresh the CDN catalog; no separate upload or duplicate files are needed. Follow the [preset maintenance rules](packages/provider-registry/README.md). At each startup, the desktop app fetches `https://www.codex-switch.com/registry/models-v1.json` in the background and compares `version`. An equal or older remote version skips merging and cache writes, including the first launch when both versions are `1`. A newer version adds new models and defaults without replacing existing model settings, user configurations or keys, or automatically selecting or enabling models. Bundled presets and a local cache keep the app usable offline. Standard services and plans have separate recommendation lists of up to five models; saved configurations are preserved.
