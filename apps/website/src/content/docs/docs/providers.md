...
预设来源于 CC Switch 固定版本，只作为新建配置的初始值；不覆盖已保存的用户地址、协议或模型，也不代表账号拥有对应权限。
...
新增配置时填写一个 API Key，并勾选 1–20 个模型。点击「保存配置」以一个具名配置保存并回到首页，不会自动发送测试请求。先关闭 Codex Switch 后，编辑时可以增删该配置的模型，留空 Key 即保留原密钥。
