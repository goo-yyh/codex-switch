...
文档站通过 `/downloads/v0.3.0/` 直接提供安装包和 `SHA256SUMS.txt`，用户下载不跳转 GitHub。`packages/product-info/downloads.json` 固定已发布版本及每个文件的 SHA-256；网站构建先运行 `scripts/prepare_downloads.mjs`，从对应 GitHub Release 下载并校验后复制到静态输出，由 Vercel CDN 提供。下载失败或哈希不符会中止构建，避免发布缺失或错误文件。

新版本须先完成 GitHub Release，再更新下载清单中的版本和全部哈希、安装说明并部署网站。已校验的本地缓存可以复用，安装包不提交到 Git；仅提高应用版本不会提前切换站点下载版本。

下载清单的 `previous` 保留已发布旧版及其哈希；网站重新部署时也会携带旧版文件，已分享的版本下载地址继续可用。
